...
function DraggableComponent({ component }: { component: ComponentItem }) {
  ...
  return (
    <div
      ref={drag}
      className={`p-2 border rounded-lg cursor-move hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 ${
...
function DroppedComponentRenderer({
  ...
  return (
    <div
      className={`absolute border rounded-lg bg-white dark:bg-gray-900 shadow-sm select-none ${
...
function DocumentPage({
  ...
  return (
    <div
      ...
      className={`relative  bg-white border border-gray-300 shadow-lg mx-auto mb-8 print:shadow-none print:border-gray-400 ${
        !isPreviewMode && isOver ? "border-blue-400 bg-blue-50" : ""
      } ${isPreviewMode ? "" : ""}`}
...
export default function TemplateBuilder() {
  ...
  const handleImageUpload = (file: File) => {
    ...
    reader.onload = (e) => {
      const result = e.target?.result as string;
      setEditContent((prev) => ({ ...prev, src: result }));
    };
    reader.readAsDataURL(file);
  };
